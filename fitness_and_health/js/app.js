(() => {
    ...
    function initSlidersFade() {
        if (document.querySelector(".swiper-fade")) new core(".swiper-fade", {
            modules: [ Navigation, Autoplay, Pagination, EffectFade, Lazy ],
            observer: true,
            observeParents: true,
            slidesPerView: 1,
            spaceBetween: 0,
    ...
    da.init();
    window.addEventListener("scroll", (function(e) {
        if (this.scrollY > 0) document.querySelector(".header").style.backgroundColor = "white"; else document.querySelector(".header").style.backgroundColor = "transparent";
    }));
    let activeSpoller = document.querySelectorAll(".spollers__title");
    if (activeSpoller) for (let i of activeSpoller) i.addEventListener("click", (function(e) {
        if (i.closest(".spollers__item")) i.closest(".spollers__item").classList.toggle("_active");
    }));
    let blogAndMedia = document.querySelector(".blog-and-media__item");
    if (blogAndMedia) blogAndMedia.querySelector(".blog-and-media__label-new").classList.add("_new");
    let everyoneScrollElement = document.querySelectorAll(".menu__link[data-goto]");
    if (everyoneScrollElement) {
        console.log(everyoneScrollElement);
        for (let i of everyoneScrollElement) {
            i.addEventListener("click", scrollToNeed);
            function scrollToNeed(e) {
