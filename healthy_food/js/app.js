(() => {
    ...
    (() => {
        ...
        da.init();
        let trueHeader = document.querySelector(".header");
        if (trueHeader) window.addEventListener("scroll", (function(e) {
            if (this.scrollY > 0) trueHeader.style.backgroundColor = "rgba(0, 0, 0, 0.6)"; else if (0 === this.scrollY) trueHeader.style.backgroundColor = "transparent";
        }));
        let dishesMain = document.querySelectorAll(".dishes-main__dinner-set");
        if (dishesMain) for (let i of dishesMain) if (i.textContent.length > 125) i.innerHTML = i.textContent.slice(0, 125) + "...";
        let menuLinks = document.querySelectorAll(".menu__link[data-goto]");
        if (menuLinks) {
            console.log(menuLinks.length);
            for (let i of menuLinks) i.addEventListener("click", (function(e) {
                if (i.dataset.goto && document.querySelector(i.dataset.goto)) {
                    const trueElemBeforeScrolling = document.querySelector(i.getAttribute("data-goto"));
                    const scrollingValue = trueElemBeforeScrolling.getBoundingClientRect().top + scrollY - document.querySelector(".header").offsetHeight - 20;
                    if (trueElemBeforeScrolling.closest(".menu-open")) document.querySelector(".menu-open").classList.remove("menu-open", "lock");
                    scrollTo({
                        top: scrollingValue,
                        behavior: "smooth"
                    });
